...
import { API_PREFIX, MEDIA_PREFIX, MAX_CHAT_BYTES, MAX_JSON_BYTES, MAX_STREAM_SECONDS } from './constants.ts'
...
export function makeAdPetRoutes(ctx: Context, service: AdPetService): WebRoute[] {
  return [
    ...
    post(ctx, `${API_PREFIX}/refresh`, MAX_JSON_BYTES, async (body) => service.refresh(typeof body.sourceId === 'string' ? body.sourceId : undefined, { locale: typeof body.locale === 'string' ? body.locale : undefined, path: typeof body.path === 'string' ? body.path : undefined, tags: Array.isArray(body.tags) ? body.tags.filter((x): x is string => typeof x === 'string') : undefined })),
    ...
    post(ctx, `${API_PREFIX}/track`, MAX_JSON_BYTES, async (body) => {
      if (typeof body.sourceId !== 'string' || typeof body.event !== 'string') throw new Error(ERRORS.missingSourceAndAction)
      if (!['impression', 'click', 'conversion'].includes(body.event)) throw new Error(ERRORS.unknownAction)
      return service.track(body.sourceId, body.event as 'impression' | 'click' | 'conversion', body.payload && typeof body.payload === 'object' ? body.payload as Record<string, unknown> : {})
    }),
    ...
    }),
    {
      kind: 'exact', path: `${API_PREFIX}/chat/stream`, handler: (req, res) => {
        ...
        readJsonBody(req, { maxBytes: MAX_CHAT_BYTES, objectOnly: true }).then(async (parsed) => {
          ...
          if (!upstream.body) throw new Error(ERRORS.streamUnavailable)
          ...
          if (!res.headersSent) writeJson(res, 400, { ok: false, error: error instanceof Error ? error.message : String(error) })
