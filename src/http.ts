 ...
 * family's route module) with drifting contracts: body caps ranging 4 KiB
 * to 1 MiB and four distinct overflow behaviors (reject, undefined, null,
 * throw).
 *
 * Edit this file in place; the host pipeline keeps it in sync via
 * `scripts/sync-shared.mjs`. No call-site changes belong in the same
 * commit as its introduction.
 * @module dsh-web-shared/host/http
 */

import type { IncomingMessage, OutgoingHttpHeaders, ServerResponse } from 'node:http'
import { JSON_BODY_MAX_BYTES } from './constants.ts'
import { ERRORS } from './messages.ts'

/** Default body cap for readJsonBody: 64 KiB. */
const DEFAULT_JSON_BODY_MAX_BYTES = JSON_BODY_MAX_BYTES
...
export async function readBoundedJson(req: IncomingMessage, maxBytes: number): Promise<unknown> {
  ...
  for await (const chunk of req) {
    ...
    size += buffer.length
    if (size > maxBytes) throw new Error(ERRORS.bodyTooLarge)
    chunks.push(buffer)
  }
