...
function headersFor(source: AdSourceConfig): Headers {
...
function buildRequest(source: AdSourceConfig, request = source.request, context: Record<string, unknown> = {}) {
  const url = ensureUrl(renderTemplate(request?.path ?? '/', context) as string, source)
...
export class AdPetService {
  ...
  async refresh(sourceId?: string): Promise<AdSnapshot> {
    if (!this.config.enabled) return this.snapshot
    const source = this.source(sourceId)
    if (!source || source.enabled === false) return { fetchedAt: Date.now(), items: [] }
    try {
  ...
  async chat(sourceId: string, payload: { message: string; history?: unknown[]; assistantId?: string; locale?: string }): Promise<AdChatResult> {
    const source = this.sourceMap.get(sourceId)
